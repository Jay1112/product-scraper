...
const  ProductItem : React.FC<ProductItemProps> = ({
    ...
    const { responseData, isCompleted, isSuccess, sendPOSTRequest, error } : {
        responseData : any,
        isCompleted : boolean,
        isSuccess : boolean,
        sendPOSTRequest : any,
    ...
    useEffect(()=>{
        if(responseData){
            setItemData(responseData?.data?.at(0));
        }
    },[responseData]);

    useEffect(()=>{
        sendPOSTRequest('/api/product',{},{},{ urls : productKey });
    },[]);
    ...
    return (
        ...
        {
            !isCompleted &&
            <div className="py-4 my-2 w-full flex items-center justify-center">
        ...
        {
            isCompleted && !isSuccess && error &&
            <p className="w-full mt-2 text-xl text-center text-red-600 dark:text-red-400">{ error?.message }</p>
        }
        {
            isCompleted && isSuccess && responseData && 
